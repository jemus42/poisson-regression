...
# Plotting
library(ggplot2)
if (!requireNamespace("firasans")) {
   remotes::install_github("hrbrmstr/firasans")
}
library(firasans)
...
Daten aus @hilbeModelingCountData2014 sind verfügbar als CSV (`HILBE-MCD-CVS-data`) auf [der Website des Autors](https://works.bepress.com/joseph_hilbe/58/).

Davon verwendet wurden:

- `azprocedure`:
    - `R` package `COUNT`, access via `data(azprocedure, package = "COUNT")`
    - Dokumentation: `?COUNT::azprocedure`
