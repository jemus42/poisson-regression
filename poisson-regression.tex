...
\else % if luatex or xetex
  ...
  \setmonofont[Mapping=tex-ansi,Scale=0.8]{Fira Mono}
...
\author{Lukas Burk}
\date{Stand: 04. December 2019 17:05 Uhr (CET)}
...
\protect\hypertarget{def:defnb2alpha}{}{\label{def:defnb2alpha} \iffalse (Negative Binimialverteilung 2, alpha) \fi{} }Nach \citep[ p.~130]{hilbeModelingCountData2014}:
...
\protect\hypertarget{def:defnb2theta}{}{\label{def:defnb2theta} \iffalse (Negative Binomialverteilung 2, theta) \fi{} }Nach \citet{perumean-chaneyZeroinflatedOverdispersedWhat2013} (p.~1675):
...
Ein Anwendungsbeispiel der ZIP findet sich auf der Seite der UCLA (\href{https://stats.idre.ucla.edu/sas/dae/zero-inflatedpoisson-regression/}{SAS}, \href{https://stats.idre.ucla.edu/r/dae/zip/}{R}, zusätzlich gibt es einmal \href{https://stats.idre.ucla.edu/sas/output/zero-inflated-poisson-regression/}{annotated Output (SAS)}), wobei der \texttt{fish}-Datensatz verwendet wird.

In diesem Datensatz beobachten wir Nullen mit zwei unterschiedlichen Ursprüngen: Zum Einen haben wir die Nullen der Personen, die geangelt haben, aber keine Fische fingen -- und zum Anderen haben wir die Nullen der Personen, die \emph{nicht angelten}, und daher garantiert auch keine Fische fingen (\enquote{\emph{certain zeros}}) (vgl. auch Abschnitt \ref{zi-modelling}).
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# ZP basierend auf fish-Daten, Modell aus UCLA-Tutorialseite}
\NormalTok{m_zip <-}\StringTok{ }\NormalTok{pscl}\OperatorTok{::}\KeywordTok{zeroinfl}\NormalTok{(count }\OperatorTok{~}\StringTok{ }\NormalTok{child }\OperatorTok{+}\StringTok{ }\NormalTok{camper }\OperatorTok{|}\StringTok{ }\NormalTok{persons, }\DataTypeTok{data =}\NormalTok{ fish)}

\KeywordTok{summary}\NormalTok{(m_zip)}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{meansim <-}\StringTok{ }\DecValTok{2}

\NormalTok{tibble}\OperatorTok{::}\KeywordTok{tibble}\NormalTok{(}
  \DataTypeTok{counts =} \KeywordTok{rpois}\NormalTok{(}\DataTypeTok{n =}\NormalTok{ nsim, }\DataTypeTok{lambda =}\NormalTok{ meansim),}
\NormalTok{) }\OperatorTok{%>%}
\StringTok{  }\KeywordTok{count}\NormalTok{(counts, }\DataTypeTok{name =} \StringTok{"Poisson"}\NormalTok{) }\OperatorTok{%>%}
\StringTok{  }\KeywordTok{mutate}\NormalTok{(}
    \DataTypeTok{Zeros =} \KeywordTok{floor}\NormalTok{(Poisson}\OperatorTok{/}\DecValTok{2}\NormalTok{),}
    \DataTypeTok{Zeros =} \KeywordTok{ifelse}\NormalTok{(counts }\OperatorTok{>}\StringTok{ }\DecValTok{0}\NormalTok{, }\DecValTok{0}\NormalTok{, Zeros)}
\NormalTok{  ) }\OperatorTok{%>%}
\StringTok{  }\KeywordTok{gather}\NormalTok{(source, freq, Poisson, Zeros) }\OperatorTok{%>%}
\StringTok{  }\KeywordTok{mutate}\NormalTok{(}\DataTypeTok{source =}\NormalTok{ forcats}\OperatorTok{::}\KeywordTok{fct_rev}\NormalTok{(source),}
         \DataTypeTok{freq =}\NormalTok{ freq}\OperatorTok{/}\NormalTok{nsim) }\OperatorTok{%>%}
\StringTok{  }\KeywordTok{ggplot}\NormalTok{(}\KeywordTok{aes}\NormalTok{(}\DataTypeTok{x =}\NormalTok{ counts, }\DataTypeTok{y =}\NormalTok{ freq, }\DataTypeTok{fill =}\NormalTok{ source)) }\OperatorTok{+}
\StringTok{  }\KeywordTok{geom_col}\NormalTok{(}\DataTypeTok{alpha =} \FloatTok{.75}\NormalTok{) }\OperatorTok{+}
\StringTok{  }\KeywordTok{scale_x_continuous}\NormalTok{(}\DataTypeTok{breaks =} \KeywordTok{seq}\NormalTok{(}\DecValTok{0}\NormalTok{, }\DecValTok{12}\NormalTok{)) }\OperatorTok{+}
\StringTok{  }\KeywordTok{scale_fill_brewer}\NormalTok{(}\DataTypeTok{palette =} \StringTok{"Set1"}\NormalTok{, }\DataTypeTok{labels =} \KeywordTok{c}\NormalTok{(}\DataTypeTok{Zeros =} \StringTok{"Certain Zeros"}\NormalTok{, }\DataTypeTok{Poisson =} \StringTok{"Poisson-Modell"}\NormalTok{)) }\OperatorTok{+}
\StringTok{  }\KeywordTok{labs}\NormalTok{(}
    \DataTypeTok{title =} \StringTok{"Schaubild: Zero-Inflated Poisson"}\NormalTok{,}
    \DataTypeTok{subtitle =} \StringTok{"Nullen in den Daten: Certain zeros + count zeros"}\NormalTok{,}
    \DataTypeTok{x =} \StringTok{"Count"}\NormalTok{, }\DataTypeTok{y =} \StringTok{"Anteil an Gesamtsichtprobe"}\NormalTok{, }\DataTypeTok{fill =} \StringTok{""}\NormalTok{,}
    \DataTypeTok{caption =} \StringTok{"Simulierte Daten"}
\NormalTok{  ) }\OperatorTok{+}
\StringTok{  }\KeywordTok{theme}\NormalTok{(}\DataTypeTok{legend.position =} \StringTok{"bottom"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\begin{center}\includegraphics{poisson-regression_files/figure-latex/zi-principle-plot-1} \end{center}
...
\hypertarget{lagrange-mulitplier-test}{%
\subsubsection{Lagrange Mulitplier Test}\label{lagrange-mulitplier-test}}

\BeginKnitrBlock{definition}[Lagrange Multiplier Test]
\protect\hypertarget{def:unnamed-chunk-3}{}{\label{def:unnamed-chunk-3} \iffalse (Lagrange Multiplier Test) \fi{} }Nach \citet{hilbeModelingCountData2014} (p.~85f):
...
\cmidrule{2-3}
 & Self-Hosted (automated build) & https://poisson.tadaa-data.de/\\
\cmidrule{2-3}
\multirow{-3}{*}{\raggedright\arraybackslash Result} & Self-Hosted (manual build) & https://lukas.tadaa-data.de/poisson/\\
\bottomrule
\end{tabular}
\end{table}

Wobei automatische builds von \href{https://travis-ci.org/jemus42/poisson-regression}{Travis CI} übernommen werden, und das Resultat an die entsprechenden Zielorte gepusht wird. Die \enquote{manuelle} Version auf \texttt{https://lukas.tadaa-data.de/poisson/} existiert primär zum schnellen Testen -- die finalen Zielorte sollen GitHub Pages und (vermutlich) \texttt{https://poisson.tadaa-data.de} sein.
...
gamlss & 5.1-5 & CRAN (R 3.6.1)\\
gamlss.data & 5.1-4 & CRAN (R 3.6.1)\\
gamlss.dist & 5.1-5 & CRAN (R 3.6.1)\\
