...
\setlength{\droptitle}{-2em}
  ...
  \postdate{\par}
    \date{Stand: 04. September 2019, 14:41 Uhr (UTC)}
...
\protect\hypertarget{def:poisson-modell}{}{\label{def:poisson-modell} \iffalse (Poisson-Modell) \fi{} }Die Zielvariablen \(y_i \in \mathbb{N}_0\) sind (bedingt) unabhängig gegeben der Kovariablen \(x_i1, x_i2, \ldots, x_ik\).
...
\item
  -\textgreater{} Underdispersion modellierbar
\end{itemize}

Eine Verteilung, die sich sowohl für over- als auch underdispersion eignet, und dabei noch gut interpretierbar bleibt.

\BeginKnitrBlock{definition}[Generalized Poisson]
\protect\hypertarget{def:def-genois}{}{\label{def:def-genois} \iffalse (Generalized Poisson) \fi{} }Aus \citet{hilbeModelingCountData2014} (p.~211), Parametrisierung nach Harris, Yang \& Harding:
...
Mit Dispersionsparameter \(\Phi = \frac{1}{(1 - \theta)^2}\).
...
\item
  ...
  \end{itemize}
\item
  R: Alternativ via \href{https://www.gamlss.com/}{gamlss}: \texttt{gamlss(...,\ family\ =\ GPO)}
\end{itemize}
...
Allgemein scheint die Parametrisierung (bzw. Notation der Parameter) sehr zu variieren.
...
In dieser Darstellung entspricht \(p\) der \emph{rate of zero-inflation} \(\pi\), \(g_1\) einer degenerierten 0-Verteilung und \(g_2\) einer Poisson-PMF.
...
\addcontentsline{toc}{subsubsection}{Tests auf Zero-Inflation}
